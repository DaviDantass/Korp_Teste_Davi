...
export class InvoiceDetailComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.billingService.getInvoice(id).subscribe({
      next: invoice => { this.invoice.set(invoice); this.loading.set(false); this.stockService.listProducts(1, 100).subscribe({ next: result => this.products.set(result.items) }); },
