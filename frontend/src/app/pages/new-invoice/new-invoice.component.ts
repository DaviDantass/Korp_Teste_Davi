...
export class NewInvoiceComponent implements OnInit {
  ...
  protected addItem(): void {
    ...
    if (!product || this.quantity < 1) {
      this.error.set('Selecione um produto e informe uma quantidade válida.');
      return;
    }
    if (this.items().some((item) => item.product.id === product.id)) {
      this.error.set('Este produto já foi adicionado à nota.');
      return;
    }
  ...
  protected createInvoice(): void {
    if (this.items().length === 0) {
      this.error.set('Adicione pelo menos um produto à nota.');
      return;
    }
    ...
    this.billingService
      ...
      .subscribe({
        ...
        error: () => {
          this.saving.set(false);
          this.error.set('Não foi possível criar a nota fiscal.');
        },
      });
  }
}
